...
// 更新实例的props属性 同时根据不同的patchFlag 进行不同处理
export function updateProps(
  instance: ComponentInternalInstance,
  rawProps: Data | null, //传入的新的props
  rawPrevProps: Data | null, //实力对象上旧的props
  optimized: boolean
) {
  const {
    props,
  ...
  const rawCurrentProps = toRaw(props) //当前的props
...
function setFullProps(
  ...
  if (rawProps) {
    for (let key in rawProps) {
      ...
      if (isReservedProp(key)) { //key ,ref等内置属性单独提取处理
        continue
      }

      if (__COMPAT__) { //针对vue2 特别处理的属性
    ...
    if (hasDefault && value === undefined) {
      ...
      if (opt.type !== Function && isFunction(defaultValue)) {
        ...
        if (key in propsDefaults) {
          ...
          setCurrentInstance(instance) //TODO
