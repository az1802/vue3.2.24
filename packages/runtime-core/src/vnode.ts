...
export function createElementBlock(
  ...
  props?: Record<string, any> | null, //vnode节点属性
  children?: any, //子节点
  patchFlag?: number,//patch 标记  patch比较更新时可以进行快速精准更新
...
// 组件vnode直接重新渲染,dom vnode需要key值夜一样才能服用
...
const normalizeRef = ({
  ...
  return (
    ref != null
      ? isString(ref) || isRef(ref) || isFunction(ref) //有效的ref属性才会被记录
...
function createBaseVNode(
  ...
  const vnode = {
    ...
    patchFlag, //path标记
    dynamicProps,//动态属性
    dynamicChildren: null, //用来存储该节点下的所有动态子节点,这样patch过程会更快
...
export function guardReactiveProps(props: (Data & VNodeProps) | null) {
  ...
  return isProxy(props) || InternalObjectKey in props
    ? extend({}, props) //浅拷贝 Object.assign 会移除响应式
    : props
}

// 克隆vnode节点 新建vnode对象 将部分属性浅拷贝至vnode属性下
// TODO 节点中ref children patchFlag这些属性的特殊处理
 ...
 * @private
 * 连续的静态节点会被合并为一个静态节点
 * _createStaticVNode("<div>aaaa</div><div>aaaa</div><div>aaaa</div><div>aaaa</div>",4)
 */
export function createStaticVNode(
  content: string, //静态节点的字符串形式
  numberOfNodes: number //创建静态节点的数量
): VNode {
...
export function normalizeVNode(child: VNodeChild): VNode {
  ...
  } else if (isArray(child)) { //多个根节点会使用Fragment节点进行一层包装
    // fragment
    return createVNode(
      Fragment,
      null,
  ...
  } else if (typeof child === 'object') { //普通节点
...
// vnode节点已经mounted则拷贝vnode返回,否则返回vnode本身
...
export function normalizeChildren(vnode: VNode, children: unknown) {
  ...
  if (children == null) {
    ...
    if (shapeFlag & (ShapeFlags.ELEMENT | ShapeFlags.TELEPORT)) {
      ...
      if (slot) { //插槽节点是render函数时,执行slot函数得到插槽子节点
        // TODO _c marker is added by withCtx() indicating this is a compiled slot
        slot._c && (slot._d = false)
        normalizeChildren(vnode, slot()) //运行插槽函数生成插槽子节点
      ...
      if (!slotFlag && !(InternalObjectKey in children!)) {
        ...
        if (
          ...
          ;(children as RawSlots)._ = SlotFlags.STABLE //稳定的插槽顺序
        } else {
          ;(children as RawSlots)._ = SlotFlags.DYNAMIC //动态插槽
