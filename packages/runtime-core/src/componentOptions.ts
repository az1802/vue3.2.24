...
//处理组件的options 执行 before created 钩子函数,完成组件实例对象的构造
export function applyOptions(instance: ComponentInternalInstance) {
  ...
  if (dataOptions) { //data必须是函数,不能是promise对象
    ...
    if (!isObject(data)) {
      ...
      if (__DEV__) {
        for (const key in data) {
          ...
          if (!isReservedPrefix(key[0])) { //_开头的变量会被仍未是私有变量,是不会挂载到ctx实例对象上的
            Object.defineProperty(ctx, key, {
              configurable: true,
              enumerable: true,
              get: () => data[key],
  ...
  if (watchOptions) { //创建watcher 监听数据的变化
  ...
  if (provideOptions) {
    const provides = isFunction(provideOptions)
      ...
      : provideOptions
    Reflect.ownKeys(provides).forEach(key => { //对自组件提供的数据均为响应式数据,所以当在自组件使用到时就会进行依赖的收集
  ...
  if (__COMPAT__) { // beforeDestroy  destroyed 转换为vue3的onBeforeUnmount onUnmounted钩子函数
    if (
      beforeDestroy &&
  ...
  if (isArray(expose)) { //将expose内的属性挂载到实例对象上
    if (expose.length) {
 ...
 * instances.
 * 解析合并的选项并将其缓存在组件上。每个组件只执行一次，因为合并不涉及实例。
 */
export function resolveMergedOptions(
  instance: ComponentInternalInstance
): MergedComponentOptions {
  const base = instance.type as ComponentOptions
  const { mixins, extends: extendsOptions } = base  //组件上朱出入的options
  const {
    mixins: globalMixins,
    optionsCache: cache, //因为options是固定的,缓存不同组件的optons,
    config: { optionMergeStrategies }
  } = instance.appContext  //全局注入options
  ...
  if (cached) {
    resolved = cached
  } else if (!globalMixins.length && !mixins && !extendsOptions) { //组件和global 均无mixins extends 混入 resolved = base
    if (
      __COMPAT__ &&
  ...
  } else {  //mergeOptions 递归处理 globalMixins 和 组件options
    resolved = {}
...
export function mergeOptions(
  ...
  if (__COMPAT__ && isFunction(from)) { //函数组件
    from = from.options
  }
